...
- **POST /api/code-agent** — только создать/обновить PR по Issue (без Reviewer).
- **POST /api/reviewer** — только прогнать Reviewer по уже созданному PR (передаёшь `repo` и `pr_number`).

Формат `repo` везде такой же: `owner/repo` или полный URL репозитория.
